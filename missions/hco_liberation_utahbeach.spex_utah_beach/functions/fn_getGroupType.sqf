...
[] call {
    ...
    // Recce vehicles count as light vehicles for display purposes
    if ((toLower _vehType) in KPLIB_b_recon_classes) exitWith {_grpType = "light";};
    if ((toLower _vehType) in KPLIB_b_medical_classes) exitWith {_grpType = "light";};
