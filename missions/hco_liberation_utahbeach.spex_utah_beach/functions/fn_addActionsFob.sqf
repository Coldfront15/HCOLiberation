...
if ((typeOf _obj) in [FOB_box_typename, FOB_truck_typename]) exitWith {
    _obj addAction [
        ...
        true,
        "",
        "isNull (objectParent _this) && {!FOB_build_in_progress} && {player getVariable ['KPLIB_hasDirectAccess', false] || {getPlayerUID player in libBuild}} && {player getVariable ['KPLIB_isAwayFromStart', false]} && {!(surfaceIsWater getPos player)}",
        10
    ];
    true
};
