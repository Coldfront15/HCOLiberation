...
if (_precise) then {
    ...
    while {_spawnPos isEqualTo []} do {
        _i = _i + 1;
        if (_i isEqualTo 16) exitWith {};
        _spawnpos = (_pos getPos [random 150, random 360]) findEmptyPosition [10, 100, _classname];
        if(!(_spawnpos isEqualTo [])) then {

            private _notEmptyEnough = _spawnpos isFlatEmpty [3, -1, -1, -1, 0, false] isEqualTo [];

            if(_notEmptyEnough) then {
                _spawnpos = [];
            };
        };
...
if (_classname in opfor_choppers || _isAir) then {


    _spawnpos = _pos vectorAdd [-500 + random 1000, -500 + random 1000, 400 + random 200];
    _newvehicle = createVehicle [_classname, _spawnpos, [], 0, 'FLY'];
    _newvehicle setPos _spawnpos;
    _newvehicle flyInHeight 500;
