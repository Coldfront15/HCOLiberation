...
if ( GRLIB_endgame == 0 && (_sector in blufor_sectors) ) then {
    if ( _attacktime <= 1 && ( [markerpos _sector] call KPLIB_fnc_getSectorOwnership == GRLIB_side_enemy ) ) then {
        ...
        [
            ...
            [
                [
                    format ["The enemy has captured %1 ", markerText _sector], // Player has joined/left the server
                    "Get back in the fight and LIBERATE!", // desc of some kind or skip?
                    "",
                    "",
                    true,
                    "",
                    "https://i.imgur.com/7TqpmMX.jpeg",
                    [],
                    [],
                    []
                ]
