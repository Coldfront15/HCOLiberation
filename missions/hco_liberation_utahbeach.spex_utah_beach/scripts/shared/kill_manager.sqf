...
if (isServer) then {
    ...
    if ((side _killer) == GRLIB_side_friendly) then {
        ...
        if (count _nearby_bigtown > 0) then {
            ...
            if(combat_readiness < 125) then {
                private _readinessIncrease = 0.01;

                
                if(_unit isKindOf "Man") then {
                    _readinessIncrease = 0.1;
                } else {
                    _readinessIncrease = 1;
                };
                combat_readiness = combat_readiness + _readinessIncrease;
                stats_readiness_earned = stats_readiness_earned + _readinessIncrease;
            };
        };
    ...
    // Player was killed
    if (isPlayer _unit && side _killer == GRLIB_side_friendly) then {
        ...
        if (isPlayer _killer && _killer != _unit) then {
            [_killer, _unit] remoteExec ["KPLIB_fnc_bluforKillWarning", _killer];
        };
    };
            ...
            if (side _killer == GRLIB_side_friendly) then {
                ...
                if (_type in KPLIB_o_inf_classes || _type in militia_squad_lower) then {
                    ...
                    [
                        ...
                        [
                            [
                                format ["%1 has murdered a surrendering soldier!", name _killer], // Player has joined/left the server
                                "War, war never changes.", // desc of some kind or skip?
                                "",
                                "",
                                true,
                                "",
                                "https://i.imgur.com/n47EqTG.jpeg",
                                [],
                                [],
                                []
                            ]
                ...
                if (_type in civilians_lower) then {
                    ...
                    [
                        ...
                        [
                            [
                                format ["%2 has murdered %1 a civilian!", name _unit, name _killer], // Player has joined/left the server
                                "The atrocities of war are astounding. You should be ashamed of yourself.", // desc of some kind or skip?
                                "",
                                "",
                                true,
                                "",
