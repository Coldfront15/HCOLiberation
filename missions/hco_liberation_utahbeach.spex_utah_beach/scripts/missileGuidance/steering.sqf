...
IADS_MissileSteering = {
	params ["_deltaTime", "_commandedAcceleration", "_missile", "_flightParams", "_guidanceState", "_accumulator", "_missileID", "_navType"];
	...
	if (!isGamePaused && accTime > 0) exitWith {
        _guidanceState params ["_yaw", "_roll", "_pitch"];
