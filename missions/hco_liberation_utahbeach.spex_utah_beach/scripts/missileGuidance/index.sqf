...
[] call compileFinal preprocessFileLineNumbers "scripts\missileGuidance\laws\index.sqf";
[] call compileFinal preprocessFileLineNumbers "scripts\missileGuidance\steering.sqf";
[] call compileFinal preprocessFileLineNumbers "scripts\missileGuidance\missileInfo.sqf";
[] call compileFinal preprocessFileLineNumbers "scripts\missileGuidance\missiles\index.sqf";
[] call compileFinal preprocessFileLineNumbers "scripts\missileGuidance\guidanceComputer.sqf";



IADS_CMHandler = {
	
	params ["_missileID", "_chaff", "_flare"];

	private _info = IADS_ActiveMissiles get _missileID;
	
	private _isIR = _info get "isIR";
	private _isRadar = _info get "isRadar";
	private _isEO = _info get "isEO";

	private _goForFlareProbability = _info getOrDefault ["goForFlareProbability", 5];
	private _goForChaffProbability = _info getOrDefault ["goForChaffProbability", 5];

	if(_isIR == true && _flare == true) then {

		private _chance = random 100;

		if(_chance < _goForFlareProbability) then {
			_info set ["cmSuccess", true];
		};
	};


	if(_isRadar == true && _chaff == true) then {
		private _chance = random 100;

		if(_chance < _goForChaffProbability) then {
			_info set ["cmSuccess", true];
		};
	};
...
IADS_SAMOverride = {
	...
	if(IADS_SAM_DEBUG == true) then {
		diag_log format ["Ammo fired %1, missile target %2, missile target time server %3", _ammo, _missileTargetDesired, missileTarget _projectile];
	};
	
	if(!(isNil "_missileTargetDesired") && !(isNull _missileTargetDesired)) then {

		private _missileType = IADS_AmmoToMissile get _ammo;


		if(!(isNil "_missileType")) then {
			[_missileType, _projectile, _missileTargetDesired, _vehicle] call IADS_HookGuidanceLogic;
		};
	};	
...
IADS_CMUsedHandler = {
	...
	if(!(isNull _vehicle)) then {
		...
		private _lowA = toLower _ammo;

		private _chaff = false;

		private _flare = false;

		if(_lowA find "chaff" != -1) then {
			_chaff = true;
		};

		if(_lowA find "flare" != -1) then {
			_flare = true;
		};

		if(_lowA find "mixed" != -1) then {
			if(_lowA find "cm" != -1) then {
				_chaff = true;
				_flare = true;
			};
		};
		...
		private _missiles = _vehicle getVariable ["missileList", []];
		{
			[_x, _chaff, _flare] call IADS_CMHandler;
		} forEach _missiles;
	};
};
...
IADS_HandleFiredEvent = {
	...
	if(!isNull _projectile && local _projectile) then {
		...
		[_unit, _weapon, _muzzle, _mode, _ammo, _magazine, _projectile, _vehicle, _missileTargetDesired] call IADS_SAMOverride;
	};
	// Handle CM
	[_unit, _weapon, _muzzle, _mode, _ammo, _magazine, _projectile, _vehicle, _missileTargetDesired] call IADS_CMUsedHandler;
};
["Man", "FiredMan", { 
	params ["_unit", "_weapon", "_muzzle", "_mode", "_ammo", "_magazine", "_projectile", "_vehicle"];
	
	[_unit, _weapon, _muzzle, _mode, _ammo, _magazine, _projectile, _vehicle, missileTarget _projectile] call IADS_HandleFiredEvent;
