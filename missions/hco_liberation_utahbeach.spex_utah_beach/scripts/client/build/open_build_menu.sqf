private [ "_oldbuildtype", "_cfg", "_initindex", "_dialog", "_iscommandant", "_squadname", "_buildpages", "_build_list", "_classnamevar", "_entrytext", "_icon", "_affordable", "_affordable_crew", "_selected_item", "_link_color", "_nearfob", "_actual_fob", "_isStartBase"];

if (([ getpos player , 300 , GRLIB_side_enemy ] call KPLIB_fnc_getUnitsCount ) > 4 ) exitWith { hint localize "STR_BUILD_ENEMIES_NEARBY";};
...
while {dialog && alive player && (dobuild == 0 || buildtype == 1)} do {
    ...
    if (dobuild == 0 && _selected_item != -1 && (_selected_item < (count _build_list))) then {
        ...
        if (!KP_liberation_allow_fob_vehcile_building && !_isStartBase) then {
            if ((_build_item select 0) isKindOf "LandVehicle" || (_build_item select 0) isKindOf "Helicopter") then {
                _affordable = false;
            };
            if (!KP_liberation_allow_fixedwing_at_fobs && (_build_item select 0) isKindOf "Plane") then {
                _affordable = false;
            };
        };
        
        private _unlockReq = nil;
        // Check via 5th index for the existence of unlock requirements (class, supply, fuel, ammo, custom name, unlockedAt)
        // If exists, then that number must be exceeded in the count of blufor_military_sectors to be unlocked
        if (count _build_item >= 6) then {
            _unlockReq = _build_item select 5;
        };

        if(!isNil { _unlockReq }) then {
            _hasUnlockRequirements = true;

             // If the unlock number is below 1, its a percentage of all military sectors for the actual count they need to liberate
            if(_unlockReq < 1) then {
                _actualCount = round ((count sectors_military) * _unlockReq);
            } else {
                _actualCount = _unlockReq;
            };

           
           
            _unlockRequirementsMet = (count blufor_military_sectors) >= _actualCount;
        };
    ...
    ctrlEnable [ 120, _affordable && _unlockRequirementsMet && !(_squad_full) ];
    ctrlEnable [ 121, _affordable_crew && _unlockRequirementsMet ];
    ...
    if (!_unlockRequirementsMet) then { _link_color = "#e00000"; };
    if ( _hasUnlockRequirements ) then {
        private _countOfSectorsOwned = count blufor_military_sectors;
        ((findDisplay 5501) displayCtrl (161)) ctrlSetStructuredText parseText ( "<t color='" + _link_color + "' align='center'>(" + str _countOfSectorsOwned + "/" + str _actualCount + ") MILITARY SECTORS</t>" );
    } else {
