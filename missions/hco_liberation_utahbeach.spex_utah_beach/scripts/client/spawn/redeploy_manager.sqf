...
private _basenamestr    = "MOB Sugar";
...
while {true} do {
    ...
    camUseNVG false;
    respawn_camera = "camera" camCreate _startPosition;
    respawn_object = "Sign_Arrow_Blue_F" createVehicleLocal _startPosition;
    ...
    if (alive player && deploy == 1) then {
        if (player getVariable ["ace_dragging_isdragging", false]) then {
            private _object = player getVariable "ace_dragging_draggedobject";
            [player, _object] call ace_dragging_fnc_dropObject;
        };

        if (player getVariable ["ace_dragging_iscarrying", false]) then {
            private _object = player getVariable "ace_dragging_carriedobject";
            [player, _object] call ace_dragging_fnc_dropObject;
        };
