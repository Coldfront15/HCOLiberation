...
BATTLESPACE_TASK_FORCE_DEFAULT_TRY_SPAWN = {
	...
	// Loop through structures and spawn them first
	{
		...
		private _crew = units (createVehicleCrew _building);
	...
	if(_remainingManpower >= 2) then {
		private _nearbyHouses = (_currentLoc) nearObjects ["Building", 250];
		...
		// [_currentLoc] call KPLIB_fnc_spawnMilitaryPostSquad;
