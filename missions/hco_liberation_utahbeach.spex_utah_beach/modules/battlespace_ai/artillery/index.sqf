...
BATTLESPACE_ARTILLERY_PIECE = "SPE_leFH18";
BATTLESPACE_ARTILLERY_PIECES_PER_BATTERY = 3;
...
BATTLESPACE_ARTILLERY_SHELL = "SPE_20x_Shell_105L28_Gr38_HE";
BATTLESPACE_ARTILLERY_WP_SHELL = "SPE_20x_Shell_105L28_Gr38_NB";
...
BATTLESPACE_ARTILLERY_MINIMUM_CYCLES_TO_SWAP = 14;
BATTLESPACE_ARTILLERY_MAXIMUM_CYCLES_TO_SWAP = 20;
	...
	if(!(BATTLESPACE_ARTILLERY_LAST_COUNTER_BATTERY_DATA isEqualTo [])) then {
		...
		if((CBA_missionTime - _time) <= 1800 && (CBA_missionTime - _time) > 90 && BATTLESPACE_ARTILLERY_NETWORK_ENABLED) then {
			...
			if((_loc distance2D _vehicle) <= 300) then {
				...
				// High accuracy
				[getPosATLVisual _vehicle, objNull, 120, true] remoteExec ["BATTLESPACE_ARTILLERY_BROADCAST_TARGET", 2]; 
...
BATTLESPACE_GET_MIN_DISPERSION = {
	...
	private _dispersion = 0 max (300 - _timeInCombat) * 0.4; //normally 0.5
	
	if(!_wp) then {
		if(_timeInCombat < 85) then {
			_dispersion = _dispersion + 50; //normally 100, lowered for SPE values
		} else {
			if(_timeInCombat < 150) then {
				_dispersion = _dispersion + 20; //normally 40, lowered for SPE values
			};
		};
	} else {
		_dispersion = 30;
...
BATTLESPACE_GET_MAX_DISPERSION = {
	...
	private _dispersion = 0 max (300 - _timeInCombat) * 0.8 + 30; //normally 60, lowered for SPE values

	if(_timeInCombat < 85) then {
		_dispersion = _dispersion + 70; //normally 100, lowered for SPE values
	} else {
		if(_timeInCombat < 150) then {
			_dispersion = _dispersion + 30; //normally 40, lowered for SPE values
		};
	};

	if(_wp) then {
		_dispersion = 0 max (300 - _timeInCombat) * 0.45 + 70; //normally 100, lowered for SPE values
	};
	...
	if(count _spawnSectors > 0) then {
		...
		while { _sectorToSpawnIn == "" && (count _spawnSectors) > 0 && _costToPullFrom >= 1 } do {
			...
			private _invalids = _validSectors select {
				...
				(_mPos distance2D (_target getPos [0,0])) > 2000 || _alreadyHasArty
			};
...
BATTLESPACE_ARTILLERY_POLL_REQUESTS = {
	...
	private _lowPop = ([] call KPLIB_fnc_getPlayerCount) <= 10;
...
BATTLESPACE_ARTILLERY_DO_REQUEST = {
	...
	private _leaderPos = (leader _battery);
	private _arty_objective_position = mapGridPosition _leaderPos;
	arty_objective_position_marker = _arty_objective_position;
	publicVariable "arty_objective_position_marker";
	...
	[
		...
		[
			[
				format ["Spotter planes observed enemy artillery battery near %1", arty_objective_position_marker], // Player has joined/left the server
				"Prepare for incoming rounds!", // desc of some kind or skip?
				"",
				"",
				true,
				"",
				"https://i.imgur.com/FVBCy0h.png",
				[],
				[],
				[]
			]
	...
	private _lowPop = ([] call KPLIB_fnc_getPlayerCount) <= 25;
	...
	private _hasCobra = true;

	// {
	// 	if((typeOf _x) == "SPE_105mm_M3_WDC" && (side _x) == GRLIB_side_friendly) exitWith {
	// 		_hasCobra = true;
	// 	};
...
BATTLESPACE_ARTILLERY_OBSERVER_COROUTINE = {
	...
	if(!(local _observer)) then {
		...
		_targets = _observer targets [true, 0, [GRLIB_side_friendly], 120];
	};

	private _lowPop = ([] call KPLIB_fnc_getPlayerCount) <= 15;
