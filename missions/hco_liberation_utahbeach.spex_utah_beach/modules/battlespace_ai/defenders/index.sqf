...
BATTLESPACE_DEFENDER_GARRISON_AI = {
    ...
    // Kind of a hack to make sure they have their stuff
    {
        ...
        // Provide gear
        _x addItemToVest "SPE_Blendkorper_2H";
        for "_i" from 1 to 5 do {
            _x addItemToVest "SPE_5Rnd_792x57";
            _x addItemToVest "SPE_1Rnd_G_FLGR";
            _x addItemToUniform "ACE_fieldDressing";
        };
        for "_i" from 1 to 4 do {
            _x addItemToVest "SPE_1Rnd_G_SPRGR_30";
            _x addItemToBackpack "SPE_Blendkorper_2H";
            _x addItemToBackpack "SPE_Blendkorper_2H";
        };
        for "_i" from 1 to 10 do {
            _x addItemToBackpack "SPE_1Rnd_G_SPRGR_30";
        };
...
BATTLESPACE_DEFENDERS_CREATE_TASK_FORCES = {
    ...
    _maximumRange = 1150 min _maximumRange;
    ...
    private _garrisonCount = _remainingInfantrySquads min ceil(_infantrySquadCount * 0.45);
    _remainingInfantrySquads = 0 max (_remainingInfantrySquads - _garrisonCount);
    private _defensivePatrols = _remainingInfantrySquads min ceil(_infantrySquadCount * 0.15);
    _remainingInfantrySquads = 0 max (_remainingInfantrySquads - _defensivePatrols);
    private _patrollingSquads = _remainingInfantrySquads min ceil(_infantrySquadCount * 0.25);
    _remainingInfantrySquads = 0 max (_remainingInfantrySquads - _patrollingSquads);
    private _ambushSquads = _remainingInfantrySquads min ceil(_infantrySquadCount * 0.15);
    ...
    private _spawnRotaryPatrol = (random 100) <= 15;
    if(_spawnRotaryPatrol && _playerCount > 10) then {
        private _composition = createHashMapFromArray [
            ["manpower", 0],
            ["vehicles", ["SPE_FW190F8"]],
            ["structures", []]
        ];
        ...
        ["Air Patrol", _composition, _pos, _objPos, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
    };
    ...
    for "_i" from 1 to _patrollingSquads do {
        ...
        private _mechanized = (random 100) < 15;
        ...
        if(_execs < 5) then {
            ["Reconnaissance Patrol", _composition, _position, _position, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
        };
    };
    ...
    if(_outpostsCount > 0) then {
        for "_i" from 1 to 2 do {
            ...
            while { 
                ...
                {
                    if((_x distance2D _pos) <= 300) exitWith { _invalid = true };
                } forEach _positions;
            ...
            if(_execs < _outpostsCount) then {
                ...
                _vehicles pushBack "SPE_MG34_Lafette_Deployed";

                // Add a bunker for cover
                _structures pushBack (createHashMapFromArray [
                    ["position", _pos],
                    ["rotation", false],
                    ["className", "Land_SPE_Sandbag_Nest"]
                ]);

                // // Whether to include an AGS
                // if((random 100) <= 90) then {
                //     _structures pushBack (createHashMapFromArray [
                //         ["position", _pos vectorAdd [0,0,4.309]],
                //         ["rotation", true],
                //         ["className", "SPE_MG34_Lafette_Deployed"],
                ...
                ["Outpost", _composition, _pos, _pos, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
            };
        };
    };
    ...
    if(_overlookCount > 0) then {
        for "_i" from 1 to _ambushSquads do {
            ...
            if(_execs < _overlookCount) then {
                _positions pushBack _pos;
                private _staticClass = "SPE_GER_R200_MG34";
                private _composition = createHashMapFromArray [
                    ["manpower", round(BATTLESPACE_SQUAD_SIZE + random(BATTLESPACE_SQUAD_SIZE))],
                    ["vehicles", []],
                    ["structures",
                ...
                ["Ambush Patrol", _composition, _pos, _pos, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
            };
        };
    };
    ...
    for "_i" from 1 to _defensivePatrols do {
        ...
        // TODO: Make it more intelligent and go towards BLUFOR sectors
        private _position = _objPos getPos [100 + random 450, random 360];
        ...
        if(_execs < 25) then {
            ["Defensive Patrol", _composition, _position, _position, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
        };
    };
    ...
    for "_i" from 1 to _garrisonCount do {
        ...
        if(_execs < 25) then {
            ...
            private _position = _objPos getPos [random 400, random 360];

            ["Garrison", _composition, _position, [], _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
        };
    };
    ...
    for "_i" from 1 to _numberOfStatics do {
        ...
        if(_execs < _potentialSpawnsCount) then {
            ...
            if((isNil { _spawnPoint }) || (_spawnPoint isEqualTo [])) then {
                ...
                // ]); // original method of spawning
                _statics pushback _vehicleClass; // fix for non-structure statics spawning in as their config side
                _positions pushBack _pos;
            };
        };
    };
    private _comp = createHashMapFromArray [
        ["manpower", 6],
        ["vehicles", _statics],
        ["structures", []] // move statics back to here if you disable the spawning side fix
    ];
    ["Fortifications", _comp, _objPos, [], _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
            ...
            if(_execs < _minePlacesCount) then {
                ...
                for "_i" from -_gridSize to _gridSize do {
                    for "_n" from -_depth to _depth do {
                        ...
                        _structs pushBack createHashMapFromArray [
                            ["position", _minePos],
                            ["rotation", 0],
                            ["className", ["SPE_SMI_35_1_MINE", "SPE_SMI_35_Pressure_MINE"] select _isAT]
                        ];
                    };
                };
                ...
                for "_i" from -1 to 1 step 0.25 do {
                    ...
                    _structs pushBack createHashMapFromArray [
                        ...
                        ["className", "SPE_Sign_Ger_Minefield"]
                    ];
                };
    ...
    for "_i" from 1 to _aaCount do {
        ...
        private _vehicles = ["SPE_FlaK_38"];

        if((random 100) <= 65) then {
            _vehicles pushBack "SPE_FlaK_36_AA";
        };
        ...
        if(_execs < _aaPlacesCount) then {
            _positions pushBack _pos;
            ["Anti-Air", _aaComp, _pos, _pos, _objPos, GRLIB_side_enemy] remoteExec ["BATTLESPACE_TASK_FORCES_INIT", 2];
        };
    };
