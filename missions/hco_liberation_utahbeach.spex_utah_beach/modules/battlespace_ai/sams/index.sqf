...
BATTLESPACE_EVALUATE_AIRSPACE = {
	...
	if(_detected != "") then {
		...
		private _spawnSectors = [blufor_sectors, 1] call NETWORKED_SECTORS_GET_SECTORS_UP_TO_COST; //Normally 7, reduced to 1 for SPE
		...
		if(count _spawnSectors > 0) then {
			...
			while { _sectorToSpawnIn == "" && (count _spawnSectors) > 0 && _costToPullFrom >= 1 } do {
				...
				private _invalids = _validSectors select {
					...
					(_mPos distance2D (getMarkerPos _detected)) > 1500
				};
...
BATTLESPACE_SAM_SITE_CREATE = {
	...
	private _units = [];
	{
		...
		_fcrGrp deleteGroupWhenEmpty true;
...
if(isServer && BATTLESPACE_ENABLE_SAM_SPAWNS) then {
	...
	if (BATTLESPACE_USE_SAM_SPAWN_DELAY) then {
		BATTLESPACE_SAM_DELAY = [1200,3200] call BIS_fnc_randomInt;
	};
